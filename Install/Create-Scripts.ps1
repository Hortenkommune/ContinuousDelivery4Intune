...
foreach ($i in $ScriptsToCreate) {
    ...
    $newInstallScript | Out-File "$PSScriptRoot\Install-CDforIntune\Install-CDforIntune.$($i.Name).ps1" -Encoding utf8BOM

    $newScript = $Script.Replace("`$BranchName = `"`"", "`$BranchName = `"$($i.Name)`"")
    $newScript = $newScript.Replace("`$Version = `"`"", "`$Version = `"$($i.Version)`"")
    $newScript | Out-File "$PSScriptRoot\CDforIntuneScript\Script.$($i.Name).ps1" -Encoding utf8BOM
}