$CustomExec = @(
    ...
    @{
        ...
        FilesToDwnload = @(
            @{
                FileName = "PEROLESSCRIPT.ps1"
                URL      = "https://raw.githubusercontent.com/Hortenkommune/ContinuousDelivery4Intune/master/resources/scripts/Run-PER.ps1"
            }
        )
        Execution      = @(
            @{
                Execute   = "powershell.exe"
                Arguments = "-ExecutionPolicy Bypass -File C:\Windows\Temp\Run-PER.ps1"
            }
        )
        Detection      = @(
            @{
