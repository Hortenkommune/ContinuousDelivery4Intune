$regfiles = @(
    ...
    @{
        URL       = "https://raw.githubusercontent.com/Hortenkommune/ContinuousDelivery4Intune/master/resources/regfiles/DisableEdgeShortcutCreation.reg"
        detection = "[bool]((Get-ItemPropertyValue -Path `"HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer`" -Name DisableEdgeShortcutCreation) -eq 1)"
        Type      = "HKLM"
    }
)
